 ...
 * TxByActionKeys
 */
class TxByActionKeys extends Interceptor {
  def this(actionKeys: String*) {
    this()
    if (actionKeys == null || actionKeys.length == 0) throw new IllegalArgumentException("actionKeys can not be blank.")
    for (actionKey <- actionKeys) actionKeySet.add(actionKey.trim)
  }
  ...
  private var actionKeySet: Set[String] = new HashSet[String]
}
