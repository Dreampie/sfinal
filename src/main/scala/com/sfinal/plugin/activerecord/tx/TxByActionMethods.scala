 ...
 * TxByActionMethods
 */
class TxByActionMethods extends Interceptor {
  def this(actionMethods: String*) {
    this()
    if (actionMethods == null || actionMethods.length == 0) throw new IllegalArgumentException("actionMethods can not be blank.")
    for (actionMethod <- actionMethods) actionMethodSet.add(actionMethod.trim)
  }
  ...
  private var actionMethodSet: Set[String] = new HashSet[String]
}
